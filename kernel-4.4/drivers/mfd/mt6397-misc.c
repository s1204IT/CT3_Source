...
static int rtc_mark_kernel_panic(struct notifier_block *self,
			    ...
			    void *data)
{
	dev_err(rtc_misc->dev, "[LY]rtc_mark_kernel_panic!!!\n");
...
{
	dev_err(rtc_misc->dev, "[LY]rtc_mark_aee_kernel_panic!!!\n");
	mtk_set_kernel_panic_reg();
	return 0;
}
					...
					unsigned long code, void *unused)
{
	...
	dev_err(rtc_misc->dev, "[LY]rtc_mark_kernel_reboot!!!\n");
...
void rtc_set_hicharge(int on)
{
	mutex_lock(&rtc_misc->lock);
	__mtk_misc_set_spare_register(RTC_HICHARGE, on);
	mutex_unlock(&rtc_misc->lock);
}

u32 rtc_get_hicharge(void)
{
	u32 ret;

	mutex_lock(&rtc_misc->lock);
	ret = __mtk_misc_get_spare_register(RTC_HICHARGE);
	mutex_unlock(&rtc_misc->lock);
	return ret;
}
...
{
	mutex_lock(&rtc_misc->lock);
	__mtk_misc_set_spare_register(RTC_FORCE_POWEROFF, 0x1);
	mutex_unlock(&rtc_misc->lock);
}
...
static void mt_power_off(void)
{
	...
	ret = regmap_read(rtc_misc->regmap, rtc_misc->addr_base + RTC_PDN1, &pdn1);
	if (ret < 0)
		dev_err(rtc_misc->dev, "regmap read error!!!\n");

	if(pdn1 & RTC_PDN1_PWRON_TIME)
		bbpu = RTC_BBPU_KEY | RTC_BBPU_AUTO | RTC_BBPU_PWREN;
	else
		bbpu = RTC_BBPU_KEY | RTC_BBPU_AUTO;
