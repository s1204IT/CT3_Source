...
int priv_driver_set_nvram(IN struct net_device *prNetDev, IN char *pcCommand, IN int i4TotalLen)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	P_ADAPTER_T prAdapter = NULL;
	...
	DBGLOG(REQ, INFO, "++priv_driver_set_nvram++\n");

	/* "SET_NVRAM offset value" */
	ASSERT(prNetDev);
	if (GLUE_CHK_PR2(prNetDev, pcCommand) == FALSE)
		return -1;
	...
	if (i4Argc >= 3) {
		...
		offset = (UINT_8)u4Parse;

		u4Ret = kalkStrtou32(apcArgv[2], 0, &u4Parse);
		u2Data = (UINT_16)u4Parse;

		DBGLOG(REQ, INFO, "priv_driver_set_nvram offset=%d, data=0x%04x\n", offset, u2Data);
	} else {
		DBGLOG(REQ, INFO, "priv_driver_set_nvram cmd error\n");
		DBGLOG(REQ, INFO, "--priv_driver_set_nvram--\n");
		return -1;
	}

	/* change to byte offset, set WIFI_NVRAM_FILE_NAME */
	...
	u2oldData = *((UINT_16 *)((PUINT_8)prRegInfo + OFFSET_OF(REG_INFO_T, aucMacAddr) - 4 + offset));
	DBGLOG(REQ, INFO, "mac_pos=%ld, pos=%ld, old_regInfo=0x%04x\n",
		OFFSET_OF(REG_INFO_T, aucMacAddr),
		OFFSET_OF(REG_INFO_T, aucMacAddr) - 4 + offset,
		u2oldData);

	/* set REG_INFO_T structure */
	*((UINT_16 *)((PUINT_8)prRegInfo + OFFSET_OF(REG_INFO_T, aucMacAddr) - 4 + offset)) = u2Data;

	DBGLOG(REQ, INFO, "set to 0x%04x\n",
		*((UINT_16 *)((PUINT_8)prRegInfo + OFFSET_OF(REG_INFO_T, aucMacAddr) - 4 + offset)));
...
int priv_driver_get_nvram(IN struct net_device *prNetDev, IN char *pcCommand, IN int i4TotalLen)
{
	P_GLUE_INFO_T prGlueInfo = NULL;
	P_ADAPTER_T prAdapter = NULL;
	P_REG_INFO_T prRegInfo = NULL;
	...
	DBGLOG(REQ, INFO, "++priv_driver_get_nvram++\n");
	...
	if (i4Argc >= 2) {
		...
		DBGLOG(REQ, INFO, "priv_driver_get_nvram offset=%d\n", offset);
	} else {
		DBGLOG(REQ, INFO, "priv_driver_set_nvram cmd error\n");
		DBGLOG(REQ, INFO, "--priv_driver_get_nvram--\n");
		return -1;
	...
	/* change to byte offset, read WIFI_NVRAM_FILE_NAME */
	...
	if (fgStatus == FALSE)
		...
		DBGLOG(REQ, INFO, "NVRAM Read u2Data=0x%04x\n", u2Data);

	DBGLOG(REQ, INFO, "--priv_driver_get_nvram--\n");
	i4BytesWritten = snprintf(pcCommand, i4TotalLen, "offset=0x%x, Data=0x%04x\nExec result OK",
		offset, u2Data);
	return i4BytesWritten;
}
...
int priv_driver_set_pwr_offset(IN struct net_device *prNetDev, IN char *pcCommand, IN int i4TotalLen)
{
	...
	/* "SET_PWR_OFFSET L-offset M-offset H-offset All-offset" */
	ASSERT(prNetDev);
	if (GLUE_CHK_PR2(prNetDev, pcCommand) == FALSE)
		return -1;
	...
	au4ChOffset = *(UINT_32 *) (prRegInfo->aucEFUSE + 28);
	...
	*((INT_32 *) ((prRegInfo->aucEFUSE + 28))) = au4ChOffset;

	/* set WIFI_NVRAM_FILE_NAME */
	u4OffsetPos = OFFSET_OF(WIFI_CFG_PARAM_STRUCT, aucEFUSE) + 28;
...
#define CMD_SETNVRAM		"SET_NVRAM"
#define CMD_GETNVRAM		"GET_NVRAM"
...
INT_32 priv_driver_cmds(IN struct net_device *prNetDev, IN PCHAR pcCommand, IN INT_32 i4TotalLen)
{
	...
	if (i4CmdFound == 0) {
		...
		else if (strncasecmp(pcCommand, CMD_SETNVRAM, strlen(CMD_SETNVRAM)) == 0)
			i4BytesWritten = priv_driver_set_nvram(prNetDev, pcCommand, i4TotalLen);
		else if (strncasecmp(pcCommand, CMD_GETNVRAM, strlen(CMD_GETNVRAM)) == 0)
			i4BytesWritten = priv_driver_get_nvram(prNetDev, pcCommand, i4TotalLen);
