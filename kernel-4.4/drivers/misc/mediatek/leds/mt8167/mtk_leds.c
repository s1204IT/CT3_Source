...
struct cust_mt65xx_led *get_cust_led_dtsi(void)
{
	...
	if (pled_dtsi == NULL) {
		...
		pled_dtsi =
		    kmalloc(MT65XX_LED_TYPE_TOTAL * sizeof(struct cust_mt65xx_led), GFP_KERNEL);
		if (pled_dtsi == NULL) {
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	/*LEDS_DEBUG("mt_mt65xx_led_set_cust() - start  !!!!!\n"); */
	...
	case MT65XX_LED_MODE_PMIC:
		{
			LEDS_DEBUG("brightness_set_cust:  MT65XX_LED_MODE_PMIC!!!!!\n");
	...
	case MT65XX_LED_MODE_CUST_EXT:
		{
			LEDS_DEBUG("mt_cust_set_led_brightness:  MT65XX_LED_MODE_CUST_EXT!!!!!\n");
			return mt_cust_set_led_brightness(cust->data, level);
		}
...
void mt_mt65xx_led_set(struct led_classdev *led_cdev, enum led_brightness level)
{
	...
	LEDS_DEBUG("---- mt_mt65xx_led_set() start!!!\n");
			...
			unsigned long *delay_on, unsigned long *delay_off)
{
	...
	LEDS_DEBUG(" ---- mt_mt65xx_blink_set() -- start!!!!!\n");
	LEDS_DEBUG(" ---- delay_on = %lu, delay_off= %lu\n", *delay_on, *delay_off);
	LEDS_DEBUG(" ---- led_data->delay_on = %d, led_data->delay_off = %d\n",
		led_data->delay_on, led_data->delay_off);
	LEDS_DEBUG(" ---- led_data->cust.mode = %d, led_data->cust.data = %ld\n",
		led_data->cust.mode, led_data->cust.data);
	...
	if (*delay_on != led_data->delay_on || *delay_off != led_data->delay_off) {
		...
		if (led_data->delay_on && led_data->delay_off) {	/* enable blink */
			...
			if (led_data->cust.mode == MT65XX_LED_MODE_PWM) {
				...
				return 0;
			} else if (led_data->cust.mode == MT65XX_LED_MODE_CUST_EXT) {
				nled_tmp_setting.nled_mode = NLED_BLINK;
				nled_tmp_setting.blink_off_time = led_data->delay_off;
				nled_tmp_setting.blink_on_time = led_data->delay_on;
				mt_cust_set_led_blink(led_data->cust.data, &nled_tmp_setting);
				return 0;
	...
	if ((*delay_on == 0) && (*delay_off == 0)) {
		if ((led_data->cust.mode == MT65XX_LED_MODE_PMIC)
			...
			|| led_data->cust.data == MT65XX_LED_PMIC_NLED_ISINK1
			|| led_data->cust.data == MT65XX_LED_PMIC_NLED_ISINK2)) {
			LEDS_DEBUG(" ---- mt_mt65xx_blink_set() -- Force disable!!!!!\n");
			mt_brightness_set_pmic(led_data->cust.data, 0, 0);
			return 0;
		}
	}

	LEDS_DEBUG(" ---- mt_mt65xx_blink_set() -- delay_on and delay_off are not changed!!!!!\n");
	return 0;
}


int mt_cust_set_led_brightness(enum mt65xx_led_cust cus_type, u32 level)
{
	LEDS_DEBUG(" ---- mt_cust_set_led() --cus_type = %d, level= %d ----\n", cus_type, level);
	return 0;
}


int mt_cust_set_led_blink(enum mt65xx_led_cust cus_type, struct nled_setting *led)
{
	LEDS_DEBUG(" ---- mt_cust_set_led_blink() --cus_type = %d ----\n", cus_type);
	LEDS_DEBUG(" ---- LED blink on time = %d offtime = %d\n", led->blink_on_time,
		led->blink_off_time);
