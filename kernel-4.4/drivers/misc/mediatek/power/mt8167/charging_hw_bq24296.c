...
static unsigned int charging_hw_init(void *data)
{
	...
	bq24296_set_vrechg(0x1); /*VRECHG 300mV*/

	bq24296_set_en_term(0x1); /*Enable termination*/
	bq24296_set_watchdog(0x0); /*WDT disable*/
	bq24296_set_en_timer(0x1); /*Enable charge timer*/
	bq24296_set_chg_timer(0x1); /*Fast Charge Timer Setting 8hrs*/

	bq24296_set_int_mask(0x3); /*Enable fault interrupt*/
...
static unsigned int charging_enable(void *data)
{
	...
	if (enable == KAL_TRUE) {
		...
		if (mt_usb_is_device()){
			...
			battery_log(BAT_LOG_CRTI, "[charging_enable] OTG ON\n");
		}
#endif
...
static unsigned int charging_get_hv_status(void *data)
{
	...
	for (i=0; i<2; i++) {
		unsigned char ret_fault = 0;
		bq24296_read_interface(bq24296_CON9, &ret_fault, 0xff, 0);
		ret_hvdet |= ret_fault;
	}
...
{
	unsigned int status = STATUS_OK;
#if defined(CONFIG_STS_CHARGER_SUPPORT)
	unsigned int det_status;

	det_status = bq24296_gpio_get_pg();
	*(kal_bool *)(data) = (det_status)?(KAL_TRUE):(KAL_FALSE);
#else
...
#elif defined(CONFIG_STS_CHARGER_SUPPORT)
