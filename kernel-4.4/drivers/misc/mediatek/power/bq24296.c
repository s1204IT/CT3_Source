...
static const struct i2c_device_id bq24296_i2c_id[] = {
   { "bq24296", 0},
   {}
};
//MODULE_DEVICE_TABLE( i2c, bq24296_i2c_id);
...
/**********************************************************
  ...
  *
  *********************************************************/
unsigned int bq24296_read_interface(
...
unsigned int bq24296_config_interface(
				    unsigned char	RegNum,
				    unsigned char	val,
				    unsigned char	MASK,
				    unsigned char SHIFT)
...
static int cpg_gpio = -1;

unsigned int bq24296_gpio_get_pg(void)
{
	unsigned int pg;
	...
	pg = !gpio_get_value(cpg_gpio); /* invert because active low */
...
static int bq24296_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	pr_err( "[%s] of_get_named_gpio( ena): %d\n", __func__, ena_gpio);
	...
	// sta_gpio
	sta_gpio	= of_get_named_gpio( client->dev.of_node, "sta-gpio", 0);
	pr_err( "[%s] of_get_named_gpio( sta): %d\n", __func__, sta_gpio);

	if( sta_gpio >= 0) {
		gpio_request( sta_gpio, NULL);
		gpio_direction_input( sta_gpio);
	}

	// set: ena_gpio
	gpio_set_value( ena_gpio, 0);	// Low: active

	cpg_gpio = of_get_named_gpio(client->dev.of_node, "cpg-gpio", 0);
	pr_err("[%s] of_get_named_gpio(cpg): %d\n", __func__, cpg_gpio);
	if (cpg_gpio >= 0) {
		gpio_request(cpg_gpio, NULL);
		gpio_direction_input(cpg_gpio);
	}
#endif /* CONFIG_OF */
...
//	bq24296_hw_component_detect();
	...
	pr_notice( "[%s] system status: 0x%02X.\n", __func__, bq24296_get_system_status());
	pr_notice( "[%s] charge config: 0x%02X.\n", __func__, bq24296_get_chg_config());
...
static ssize_t store_bq24296_access(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	battery_log( BAT_LOG_FULL, "[%s] passed.\n", __func__);
	...
	if (buf != NULL && size != 0) {
		...
		if (size > 3) {
			...
			battery_log( BAT_LOG_FULL, "[%s] write bq24296 reg 0x%x with value 0x%x !\n", __func__,
			...
			battery_log( BAT_LOG_FULL, "[%s] read bq24296 reg 0x%x with value 0x%x !\n", __func__,
			     reg_address, g_reg_value_bq24296);
			battery_log( BAT_LOG_FULL, "[%s] use \"cat bq24296_access\" to get value\r\n", __func__);
		}
	}
	return size;
}
...
static int bq24296_user_space_probe(struct platform_device *dev)
{
	...
	pr_notice( "******** [%s] passed !! ********\n", __func__);
	pr_notice( "****[%s] probe device: (%s)\n", __func__, dev->dev.init_name);
...
#ifndef CONFIG_OF
	i2c_register_board_info( BQ24296_BUSNUM, &i2c_bq24296, 1);		// BQ24296_BUSNUM == 2
#endif
