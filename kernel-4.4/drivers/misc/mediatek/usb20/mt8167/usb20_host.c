...
*      Change USB connection detection method
*      Change USB VBUS output method
...
#ifdef CONFIG_OC
static unsigned int oc_pin;
static unsigned int oc_pin_mode;
static unsigned int oc_if_config = 1;
static unsigned int cur_oc_state = 0;
struct pinctrl_state *pinctrl_oc;
static int usb_oc_number;
#endif

static unsigned int usb_detect_pin;
static int usb_detect_number;
static unsigned int cur_detect_state = 0;
...
bool	is_usb_det(void)
{
	return gpio_get_value(usb_detect_pin) ? 0:1;
}
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
	DBG(0, "mt65xx_usb20_vbus++,is_on=%d\r\n", is_on);
#ifndef FPGA_PLATFORM
	if (is_on && musb->is_self_powered == 0 ) {
...
#ifdef CONFIG_OC
static irqreturn_t mt_usb_ext_oc_int(int irq, void *dev_id)
{
	wake_lock(&mtk_musb->usb_lock);
	
	if (cur_oc_state == 0) {
		cur_oc_state = 1;
		mtk_musb->is_self_powered = 1;
		musb_platform_set_vbus(mtk_musb, 0);
		irq_set_irq_type(usb_oc_number, IRQF_TRIGGER_HIGH);
		DBG(0, "usb over current detect\n");
	} else {
		cur_oc_state = 0;
		irq_set_irq_type(usb_oc_number, IRQF_TRIGGER_LOW);
		DBG(0, "usb over current non_detect\n");
	}
...
static irqreturn_t mt_usb_detect_int(int irq, void *dev_id)
{
	if (cur_detect_state == 0) {
		cur_detect_state = 1;
		irq_set_irq_type(usb_detect_number, IRQF_TRIGGER_HIGH);
		mt_usb_connect();
		DBG(0, "usb cable connect\n");
	} else {
		cur_detect_state = 0;
		irq_set_irq_type(usb_detect_number, IRQF_TRIGGER_LOW);
		mt_usb_disconnect();
		DBG(0, "usb cable disconnect\n");
	}
	return IRQ_HANDLED;
}
...
#ifdef CONFIG_OC
static void oc_int_init(void)
{
	int ret = 0;

	DBG(0, "****%s:%d before Init OVER CURRRENT !!!!!\n", __func__, __LINE__);

	pinctrl_oc = pinctrl_lookup_state(pinctrl, "oc_irq_init");
	if (IS_ERR(pinctrl_oc)) {
		ret = PTR_ERR(pinctrl_oc);
		DBG(0, "Cannot find usb pinctrl oc_irq_init\n");
	}

	pinctrl_select_state(pinctrl, pinctrl_oc);
	DBG(0, "usb oc_pin %d\n", oc_pin);

	usb_oc_number = __gpio_to_irq(oc_pin);
	DBG(0, "usb usb_oc_number %d\n", usb_oc_number);

	cur_oc_state = 0;

	ret = request_irq(usb_oc_number, mt_usb_ext_oc_int, IRQF_TRIGGER_LOW, "USB_OC", NULL);
	if (ret > 0)
		DBG(0, "USB OC IRQ LINE not available!!\n");
	else
		DBG(0, "USB OC IRQ LINE available!!\n");

	irq_set_irq_wake(usb_oc_number, 1);
}
#endif
void mt_usb_otg_init(struct musb *musb)
{
	...
	if (node == NULL) {
		...
		//usb detect
		usb_detect_pin = of_get_named_gpio(node, "usb_detect_gpio", 0);
//		printk("%s usb_detect_gpio:%4d\n", __func__, usb_detect_pin);

		gpio_request(usb_detect_pin, NULL);
		gpio_direction_input(usb_detect_pin);
//		gpio_get_value(usb_detect_pin);

		usb_detect_number = __gpio_to_irq(usb_detect_pin);
//		printk( "usb usb_detect_number %d\n", usb_detect_number);

		cur_detect_state = 0;

		if (request_irq(usb_detect_number, mt_usb_detect_int, IRQF_TRIGGER_LOW, "USB_DETECT", NULL) > 0)
			DBG(0,"USB DETECT IRQ LINE not available!!\n");
		else
			DBG(0,"USB DETECT IRQ LINE available!!\n");

		irq_set_irq_wake(usb_detect_number, 1);
...
#ifdef CONFIG_OC
		//over current 
		oc_pin = of_get_named_gpio(node, "oc_gpio", 0);
		if (oc_pin == 0) {
			oc_if_config = 0;
			DBG(0, "oc_gpio fail\n");
		}
		oc_pin_mode = of_get_named_gpio(node, "oc_gpio", 1);
#endif
#if !defined(OTG_BOOST_BY_SWITCH_CHARGER)
...
#ifdef CONFIG_OC
	/* init over current pin interrupt */
	//INIT_DELAYED_WORK(&musb->ov_pin_work, musb_ov_pin_work);
	oc_int_init();
#endif
	/* EP table */
