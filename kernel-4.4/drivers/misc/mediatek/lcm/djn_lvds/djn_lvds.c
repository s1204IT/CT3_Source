...
static unsigned int GPIO_LCD_BIAS_EN;
...
static int lcm_get_gpio(struct device *dev)
{
	...
	GPIO_LCD_BIAS_EN = of_get_named_gpio(dev->of_node, "gpio_lcd_bias_en", 0);
	gpio_request(GPIO_LCD_BIAS_EN, "GPIO_LCD_BIAS_EN");
	pr_err("[Kernel/LCM] GPIO_LCD_BIAS_EN = 0x%x\n", GPIO_LCD_BIAS_EN);
...
static void lcm_get_params(LCM_PARAMS *params)
{
    ...
    params->dpi.hsync_pulse_width 	= 100;
    params->dpi.hsync_back_porch  	= 100;
    params->dpi.hsync_front_porch 	= 120;

    params->dpi.vsync_pulse_width 	= 12;
    ...
    params->dpi.PLL_CLOCK 		  	= 65;	// from panel spec  AUO
//  params->dpi.PLL_CLOCK 		  	= 71;	// from panel spec  Arima
...
void lcm_suspend(void)
{
	...
	lcm_set_gpio_output(GPIO_LCD_BIAS_EN, GPIO_OUT_ZERO);
	MDELAY(20);
...
void lcm_resume(void)
{
	...
	lcm_set_gpio_output(GPIO_LCD_BIAS_EN, GPIO_OUT_ONE);
	MDELAY(100);
