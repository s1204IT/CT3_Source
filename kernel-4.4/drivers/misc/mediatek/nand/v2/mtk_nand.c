					...
					u16 sec_num, bool full, u8 *buf, enum readCommand cmd)
{
	...
	if (cmd == AD_CACHE_FINAL) {
		if (!mtk_nand_set_command(0x3F)) {
			nand_err("Send 0x3F command failed!");
			goto cleanup;
		}
		if (!mtk_nand_status_ready(STA_NAND_BUSY)) {
			nand_err("STA_NAND_BUSY after 0x3F failed!");
			goto cleanup;
		}
		return true;
	}
	if (!mtk_nand_set_command(NAND_CMD_READ0)) {
		nand_err("NAND_CMD_READ0 failed!");
		goto cleanup;
	}
	if (!mtk_nand_set_address(col_addr, u4RowAddr, colnob, rownob)) {
		nand_err("mtk_nand_set_address failed!");
		goto cleanup;
	}
	if (cmd == NORMAL_READ) {
		if (!mtk_nand_set_command(NAND_CMD_READSTART)) {
			nand_err("NAND_CMD_READSTART failed!");
			goto cleanup;
		}
	} else {
		if (!mtk_nand_set_command(0x31)) {
			nand_err("send 0x31 command failed!");
			goto cleanup;
		}
	}

	if (!mtk_nand_status_ready(STA_NAND_BUSY)) {
		nand_err("check STA_NAND_BUSY failed!");
		goto cleanup;
	}
...
#endif
	if (!bRet)
		nand_err("fail!!! will return false.");
	return bRet;
}
...
static bool mtk_nand_dma_read_data(struct mtd_info *mtd, u8 *buf, u32 length)
{
	...
	if (interrupt_en) {
		...
		while ((DRV_Reg32(NFI_MASTERSTA_REG16) & 0x3) != 0) {
			timeout--;
			if (timeout == 0) {
				nand_err("poll NFI_MASTERSTA_REG16 error (0x%x)\n",
				...
				u8 *pFDMBuf)
{
	...
	if (mtk_nand_ready_for_read(nand, real_row_addr, 0, data_sector_num, true, buf, NORMAL_READ)) {
		while (logical_plane_num) {
...
#if __INTERNAL_USE_AHB_MODE__
			if (!phys)
				nand_err("convert virt addr (%p) to phys add (%x)fail!!!", temp_byte_ptr, phys);
			else
				DRV_WriteReg32(NFI_STRADDR_REG32, phys);
#endif
			...
			if (!mtk_nand_read_page_data(mtd, temp_byte_ptr,
				...
				nand_err("mtk_nand_read_page_data fail\n");
				bRet = ERR_RTN_FAIL;
			}
			if (!mtk_nand_status_ready(STA_NAND_BUSY)) {
				nand_err("mtk_nand_status_ready fail\n");
				bRet = ERR_RTN_FAIL;
			}
			if (g_bHwEcc) {
				if (!mtk_nand_check_dececc_done(data_sector_num)) {
					nand_err("mtk_nand_check_dececc_done fail 0x%x\n", u4RowAddr);
					bRet = ERR_RTN_FAIL;
				}
			}
			...
			if (host->debug.err.en_print)
				pr_err("%s, u4RowAddr:%d, max_num:%d\n", __func__,
	...
	if (retryCount != 0) {
		...
		if (bRet == ERR_RTN_SUCCESS) {
			...
			nand_err("u4RowAddr: 0x%x read retry fail, mtd_ecc(A): %x , fail, mtd_ecc(B): %x\n",
	...
	if (bRet != ERR_RTN_SUCCESS) {
		nand_err("ECC uncorrectable , fake buffer returned\n");
				   ...
				   u8 *pPageBuf, u8 *pFDMBuf, int subpageno)
{
	...
	if (bRet != ERR_RTN_SUCCESS) {
		nand_err("ECC uncorrectable, fake buffer returned,(0x%x,0x%x,%p,%p,%d,%d)\n",
	...
	if (mtk_nand_ready_for_write(chip, real_row_addr, 0, true, buf)) {
		...
		if (host->wb_cmd == NAND_CMD_PAGEPROG) {
			if (!wait_for_completion_timeout(&g_comp_Busy_Ret, msecs_to_jiffies(NFI_TIMEOUT_MS))) {
				nand_err("timeout!!!slcopmodeEn:%d, u4RowAddr:%d NFI_STA_REG32:0x%x\n",
...
#endif
		{
			if (status & NAND_STATUS_FAIL) {
				pr_err("u4RowAddr:0x%x, write fail!! status 0x%x\n", u4RowAddr, status);
				return -EIO;
			} else
				return 0;
		}
...
static u8 *mtk_nand_get_align_buf(u8 *buf)
{
	if (!local_buffer_16_align) {
		nand_err("local buffer is not init\n");
		return buf;
	}
			...
			struct mtk_nand_chip_read_param *param)
{
	...
	do {
		...
		for (i = 0; i < page_num; i++) {
			...
			if (err) {
				nand_err("wait dma ready fail, i:%d\n", i);
				err = -READ_ERR_NFI;
			}
				 ...
				 int subpage, int subpageno)
{
	...
	if (bRet == ERR_RTN_SUCCESS) {
		...
		nand_err("read err:%d, rowaddr:0x%x, coladdr:0x%x, subpage:%d, subpageno:%d\n",
