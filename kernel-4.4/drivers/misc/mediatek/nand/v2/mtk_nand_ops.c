	...
	int count)
{
	...
	if (count != info->max_keep_pages) {
		nand_err("error:count:%d max:%d", count, info->max_keep_pages);
		return 0;
	}
...
static int open_block_init(struct open_block *open)
{
	...
	if (open->array == NULL) {
		nand_err("kmalloc open array fail!!! max:%d\n", open->max);
		return -1;
	}
...
static int open_block_add(struct open_block *open, int block)
{
	...
	if (open_block_in_list(open, block)) {
		nand_err("block(%d) in open list", block);
		ret = 0;
		goto OUT;
	}
