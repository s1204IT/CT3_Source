...
static int nvram_read(char *filename, char *buf, ssize_t len, int offset)
{
	...
	if (IS_ERR(fd)) {
		pr_err("[nvram_read] : failed to open!!\n");
		return -1;
	}

	do {
		if (fd->f_op == NULL) {
			pr_err("[nvram_read] : f_op is NULL!!\n");
			break;
		}
		...
		if (fd->f_pos != offset) {
			if (fd->f_op->llseek) {
				if (fd->f_op->llseek(fd, offset, 0) != offset) {
					pr_err("[nvram_read] : failed to seek!!\n");
					break;
				}
			} else {
				fd->f_pos = offset;
		...
		if (retLen < 0)
			pr_err("[nvram_read] : read failed!! Error code: %d\n", retLen);
...
static int nvram_write(char *filename, char *buf, ssize_t len, int offset)
{
	...
	if (IS_ERR(fd)) {
		pr_debug("[nvram_write] : failed to open!!\n");
		return -1;
	}

	do {
		if (fd->f_op == NULL) {
			pr_debug("[nvram_write] : f_op is NULL!!\n");
			break;
		}
		/* End of if */
		if (fd->f_pos != offset) {
			if (fd->f_op->llseek) {
				if (fd->f_op->llseek(fd, offset, 0) != offset) {
					pr_debug("[nvram_write] : failed to seek!!\n");
					break;
				}
			} else {
				fd->f_pos = offset;
		...
		if (retLen < 0)
			pr_debug("[nvram_write] : write failed!! Error code: %d\n", retLen);
		/* write valid byte, only for 1 byte nvram block */
		pos = 2;
		retLen = __vfs_write(fd, p, sizeof(unsigned char), &pos);
		if (retLen < 0)
			pr_debug("[nvram_write] : write valid failed!! Error code: %d\n", retLen);
