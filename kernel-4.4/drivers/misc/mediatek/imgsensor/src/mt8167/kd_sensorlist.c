...
#if 1
		/* Just return, because: the feature is not ok!*/
		timeout = -1;	/* No op: for avoid build error */
		return 0;
#else
...
static inline int adopt_CAMERA_HW_GetResolution(void *pBuf)
{
	...
	PK_INFO("[CAMERA_HW] adopt_CAMERA_HW_GetResolution, pBuf: %p, resolution:%p\n",
				pBuf, pBufResolution->pResolution);

	if (access_ok(VERIFY_WRITE, pBufResolution->pResolution, sizeof(ACDK_SENSOR_RESOLUTION_INFO_STRUCT)) == 0) {
		PK_ERR("Fatal Error: pResolution can't be used!\n");
		return -EFAULT;
	}
