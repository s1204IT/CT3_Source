...
typedef struct GED_DUMP_FW_TAG {
	struct work_struct	sWork;
} GED_DUMP_FW;
...
{
	GED_DUMP_FW *psNotify = GED_CONTAINER_OF(psWork, GED_DUMP_FW, sWork);

	if (psNotify) {
		MTKFWDump();
		ged_free(psNotify, sizeof(GED_DUMP_FW));
	}
}

void ged_dump_fw(void)
{
	GED_DUMP_FW *psNotify;

	psNotify = (GED_DUMP_FW *)ged_alloc_atomic(sizeof(GED_DUMP_FW));
	if (psNotify) {
