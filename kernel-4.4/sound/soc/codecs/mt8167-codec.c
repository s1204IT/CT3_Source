...
//force gain init date
#define	HP_L_PGA_INIT_GAIN 	3	//+4db
#define	HP_R_PGA_INIT_GAIN 	3	//+4db
#define	LOUT_PGA_INIT_GAIN	9	//+0db
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	switch (id) {
	case HP_L_PGA_GAIN:
		value = HP_L_PGA_INIT_GAIN;			//force set gain
		snd_soc_update_bits(codec, AUDIO_CODEC_CON01,
			GENMASK(2, 0), value);
		break;
	case HP_R_PGA_GAIN:
		value = HP_R_PGA_INIT_GAIN;			//force set gain
		snd_soc_update_bits(codec, AUDIO_CODEC_CON01,
			GENMASK(5, 3), value << 3);
		break;
	case LOUT_PGA_GAIN:
		value = LOUT_PGA_INIT_GAIN;			//force set gain
		snd_soc_update_bits(codec, AUDIO_CODEC_CON02,
...
static int	set_force_gain(int id, struct snd_ctl_elem_value *ucontrol, int index)
{
	if(id == HP_L_PGA_GAIN){
		ucontrol->value.integer.value[index] = HP_L_PGA_INIT_GAIN;
	}else if(id == HP_R_PGA_GAIN){
		ucontrol->value.integer.value[index] = HP_R_PGA_INIT_GAIN;
	}else if(id == LOUT_PGA_GAIN){
		ucontrol->value.integer.value[index] = LOUT_PGA_INIT_GAIN;
	}else{
		return	0;
	}
	return	1;
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	set_force_gain(id, ucontrol, 0);
	if (snd_soc_volsw_is_stereo(mc) && (id+1 < PGA_GAIN_MAX))
		set_force_gain(id, ucontrol, 1);
...
static void mt8167_codec_init_regs(struct mt8167_codec_priv *codec_data)
{
	...
	/* setup default gain */
	codec_data->pga_gain[HP_L_PGA_GAIN] = HP_L_PGA_INIT_GAIN;
	codec_data->pga_gain[HP_R_PGA_GAIN] = HP_R_PGA_INIT_GAIN;
	codec_data->pga_gain[LOUT_PGA_GAIN] = LOUT_PGA_INIT_GAIN;
