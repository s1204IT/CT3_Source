...
int check_log_folder_exit()
{
    ...
    if (statfs(COMBO_T32_DIR,&fs_info) < 0)
    {
        ...
        {
            stp_printf(MSG_ERROR, "remake (%s) failed\n", COMBO_T32_DIR, strerror(errno));
            return -1;
        } else
            stp_printf(MSG_WARNING, "remake (%s) success\n", COMBO_T32_DIR);
    }
...
void wait_sdcard_ready()
{
    ...
    while (1)
    {
        ...
        {
            count++;
            stp_printf(MSG_ERROR, "fail to get Sdcard stat :%s, count: %d", strerror(errno), count);
        }
        else
        {
            if(fs_status.f_blocks==0)
            {
                stp_printf(MSG_ERROR, "wait sdcard not ready count:%d,:%s", count, strerror(errno));
            }
