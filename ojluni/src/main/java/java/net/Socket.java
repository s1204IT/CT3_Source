...
class Socket implements java.io.Closeable {
    ...
    public void connect(SocketAddress endpoint, int timeout) throws IOException {
        ...
        ///M: Support Mom Check @{
        if (isEmailSendPort(port)) {
           if (!checkEmailSendRequest()) {
        ...
        /// M: debug logging @{
        System.out.println("[socket][" + sIndex + "] connection "
    ...
    public synchronized void close() throws IOException {
        synchronized(closeLock) {
            if (isClosed())
                return;
            if (created) {
                /// M: socket debug logging.
                System.out.println(
                        "close [socket][" + localAddress + ":" + getLocalPort() + "]");
    ...
    /**
     * Check email is permitted to send or not.
     * @hide
     */
