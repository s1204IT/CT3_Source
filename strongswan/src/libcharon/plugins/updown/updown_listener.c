	...
	bool up)
{
	...
	if (!up)
	{	/* Config name: apn_xxx */
		notify_wod(N_DETACH, config->get_name(config) + 4, NULL);
	}
	else
	{
        if (this->prop.pcscf6[0][0] || this->prop.pcscf[0][0])
        {
    		notify_wod(N_PCSCF,config->get_name(config) + 4, &this->prop);
        }
        if (this->prop.dns6[0][0] || this->prop.dns[0][0])
        {
		    notify_wod(N_DNS, config->get_name(config) + 4, &this->prop);
        }
		this->prop.status = ike_sa->get_status_code(ike_sa);
		this->prop.sub_error = ike_sa->get_sub_error_code(ike_sa);
		notify_wod(N_ATTACH, config->get_name(config) + 4, &this->prop);
	}
#endif
