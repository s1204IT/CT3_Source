...
struct private_child_cfg_t {
	...
	/**
	 * replace the value in TSi with the prefix_64bits internal IPv6 address for IPv6 only, 
	 * if tsi_full and tsi_64 all enabled, apply tsi_full first and then tsi_64
	 */
	bool force_tsi_64;

	/**
	 * replace the ip address in TSi with the whole IP address
	 * (full ipv6 address and ipv4 addresss) 
	 */
	bool force_tsi_full;
};
...
METHOD(child_cfg_t, set_wod_options, void,
	private_child_cfg_t *this, char *vif, bool force_tsi_64, bool force_tsi_full)
{
	this->vif = strdupnull(vif);
	this->force_tsi_64 = force_tsi_64;
	this->force_tsi_full = force_tsi_full;
}

METHOD(child_cfg_t, get_vif, char*,
	private_child_cfg_t *this)
{
	return this->vif;
}

METHOD(child_cfg_t, use_force_tsi_64, bool,
	private_child_cfg_t *this)
{
	return this->force_tsi_64;
}

METHOD(child_cfg_t, use_force_tsi_full, bool,
	private_child_cfg_t *this)
{
	return this->force_tsi_full;
}
							  ...
							  mark_t *mark_in, mark_t *mark_out, u_int32_t tfc)
{
	...
	INIT(this,
		.public = {
			...
			.get_vif = _get_vif,
			.use_force_tsi_64 = _use_force_tsi_64,
			.use_force_tsi_full = _use_force_tsi_full,
		},
		.name = strdup(name),
