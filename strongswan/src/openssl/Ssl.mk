...
# # host shared library
# include $(CLEAR_VARS)
# LOCAL_SHARED_LIBRARIES += libcrypto-host-ss $(log_shared_libraries)
# LOCAL_MODULE_TAGS := optional
# LOCAL_CFLAGS += -DOPENSSL_HOST_LOGD
# LOCAL_MODULE := libssl-host-ss
# LOCAL_MULTILIB := both
# LOCAL_ADDITIONAL_DEPENDENCIES := $(LOCAL_PATH)/android-config.mk $(LOCAL_PATH)/Ssl.mk
...
# LOCAL_SHARED_LIBRARIES := libssl-ss libcrypto-ss $(log_shared_libraries) libcutils
# 
# ifeq ($(TARGET_BUILD_VARIANT),eng)
# LOCAL_CFLAGS += -DDEV_VERSION
# endif
# LOCAL_MODULE := ssltest-ss
# LOCAL_MULTILIB := both
# LOCAL_MODULE_STEM_32 := ssltest-ss
# LOCAL_MODULE_STEM_64 := ssltest64-ss
# LOCAL_MODULE_TAGS := optional
# LOCAL_ADDITIONAL_DEPENDENCIES := $(LOCAL_PATH)/android-config.mk $(LOCAL_PATH)/Ssl.mk
# include $(LOCAL_PATH)/Ssl-config-host.mk
# include $(LOCAL_PATH)/android-config.mk
