...
static inline bool is_multi_value_rule(auth_rule_t type)
{
	switch (type)
	{
		...
		case AUTH_RULE_MAX:
		case AUTH_HELPER_BUNDLE_HASH_URL:
			return FALSE;
...
static bool complies_orig(private_auth_cfg_t *this, 
	auth_cfg_t *constraints, bool log_error, bool skip_check_id, bool server_nocert)
{
	enumerator_t *e1, *e2;
		...
		switch (t1)
		{
			...
			case AUTH_RULE_XAUTH_IDENTITY:
			{
				...
				if (!id2 || !id2->matches(id2, id1))
				{
					...
					if (skip_check_id)
					{
						DBG1(DBG_IKE, "*** skip ID check OKOKOKOKOK ***");
						break;
					}
			...
			case AUTH_RULE_AUTH_CLASS:
			{
					...
					(uintptr_t)value != (uintptr_t)get(this, t1))
				{
					if (server_nocert)
					{
						DBG1(DBG_IKE, "*** server no cert. skip cert check OKOKOKOKOK ***");
						break;
					}
			...
			case AUTH_RULE_IDENTITY_APN:
				/* just an indication in IDr when WO S2b */
			case AUTH_RULE_IDENTITY_LOOSE:
...
{
	return complies_orig(this, constraints, log_error, FALSE, FALSE);
}

METHOD(auth_cfg_t, complies_v2, bool,
	private_auth_cfg_t *this, auth_cfg_t *constraints, bool log_error,
	bool skip_check_id, bool server_nocert)
{
	return complies_orig(this, constraints, log_error, skip_check_id, server_nocert);
}
